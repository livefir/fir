...
func (bc *BoardCreate) AddStoryIDs(ids ...uuid.UUID) *BoardCreate {
	bc.mutation.AddStoryIDs(ids...)
	return bc
}

// AddStories adds the "stories" edges to the Story entity.
func (bc *BoardCreate) AddStories(s ...*Story) *BoardCreate {
	ids := make([]uuid.UUID, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return bc.AddStoryIDs(ids...)
}
...
func (bc *BoardCreate) createSpec() (*Board, *sqlgraph.CreateSpec) {
	...
	if nodes := bc.mutation.StoriesIDs(); len(nodes) > 0 {
