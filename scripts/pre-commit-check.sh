...
# Run Chrome-based tests e2e with Docker support
run_chrome_tests() {
...
main() {
    ...
    if [ "$FAST_MODE" = true ]; then
        ...
        if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
            TEST_LABEL="Tests with Docker (Excluding E2E)"
            TEST_CMD="DOCKER=1 go test $TEST_FLAGS \$(go list ./... | grep -v -E \"examples/e2e\")"
            log "Docker available: Running full test suite excluding E2E tests with $PARALLEL_JOBS parallel processes"
        else
            TEST_LABEL="Tests (Excluding E2E)"
            TEST_CMD="go test $TEST_FLAGS \$(go list ./... | grep -v -E \"examples/e2e\")"
            log "Running full test suite excluding E2E tests with $PARALLEL_JOBS parallel processes"
        fi
    fi
