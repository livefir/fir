...
func (bq *BoardQuery) QueryTodos() *TodoQuery {
	query := &TodoQuery{config: bq.config}
...
func (bq *BoardQuery) WithTodos(opts ...func(*TodoQuery)) *BoardQuery {
	query := &TodoQuery{config: bq.config}
...
func (bq *BoardQuery) sqlAll(ctx context.Context) ([]*Board, error) {
	...
	if query := bq.withTodos; query != nil {
		...
		query.Where(predicate.Todo(func(s *sql.Selector) {
			s.Where(sql.InValues(board.TodosColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.board_todos
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "board_todos" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "board_todos" returned %v for node %v`, *fk, n.ID)
			}
